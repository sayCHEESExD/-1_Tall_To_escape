...
export class TouchControls {
  ...
  constructor(look: LookSink) {
    ...
    this.stick.className = 'hj-touch__stick';
    this.knob = document.createElement('div');
    this.knob.className = 'hj-touch__knob';
  ...
  setSuppressed(suppressed: boolean): void {
    ...
    this.root.classList.toggle('hj-touch--hidden', suppressed);
  ...
  private readonly onCanvasDown = (event: PointerEvent): void => {
    ...
    if (this.movePointer === null && this.inStickZone(event.clientX, event.clientY)) {
      ...
      this.placeStick();
      this.stick.classList.add('hj-touch__stick--active');
  ...
  private readonly onJumpDown = (event: PointerEvent): void => {
    ...
    this.jumpHeld = true;
    this.jumpPulse = true;
    this.jumpButton.classList.add('hj-touch__jump--down');
  ...
  private readonly onJumpUp = (event: PointerEvent): void => {
    event.stopPropagation();
    this.jumpHeld = false;
    this.jumpButton.classList.remove('hj-touch__jump--down');
    try {
  ...
  private readonly measure = (): void => {
    ...
    this.stick.style.setProperty('--hj-stick-radius', `${this.radius}px`);
...
.hj-touch[hidden] { display: none; }
.hj-touch--hidden { opacity: 0; pointer-events: none; }

.hj-touch__stick {
  --hj-stick-radius: 64px;
  position: fixed;
  left: calc(var(--hj-safe-l, 0px) + 26px + var(--hj-stick-radius));
  top: auto;
  bottom: calc(var(--hj-safe-b, 0px) + 26px);
  width: calc(var(--hj-stick-radius) * 2);
  height: calc(var(--hj-stick-radius) * 2);
  margin: calc(var(--hj-stick-radius) * -1) 0 0 calc(var(--hj-stick-radius) * -1);
...
.hj-touch__jump {
  position: fixed;
  right: calc(var(--hj-safe-r, 0px) + 24px);
  bottom: calc(var(--hj-safe-b, 0px) + 34px);
...
.hj-touch__jump--down { transform: translateY(4px); box-shadow: none; filter: brightness(1.12); }
`;

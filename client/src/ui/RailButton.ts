...
export class RailButton {
  ...
  constructor(
    parent: HTMLElement,
    options: {
      /** Modifier suffix: `hj-tile--<variant>` supplies the gradient. */
    ...
    this.root.className = `hj-tile hj-tile--${options.variant}`;
    this.root.setAttribute('aria-label', options.label);
    this.root.innerHTML = options.icon;

    const label = document.createElement('span');
    label.className = 'hj-tile__label hj-font hj-outline';
    label.textContent = options.label;
    this.root.appendChild(label);

    if (options.hotkey) {
      const key = document.createElement('span');
      key.className = 'hj-tile__key hj-font';
    ...
    this.badge.className = 'hj-tile__badge hj-font';
  ...
  setState(ready: boolean, locked = false): void {
    this.root.classList.toggle('hj-tile--ready', ready);
    this.root.classList.toggle('hj-tile--locked', locked);
  }

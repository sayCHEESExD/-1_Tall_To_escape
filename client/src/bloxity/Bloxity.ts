import { BLOXITY_GAME_ID } from '@highjump/shared';
import { logger } from '../util/logger.js';
import {
  DEFAULT_PROPORTIONS,
  type LegionEquipped,
  type LegionFriend,
  type LegionFriendRequestResult,
...
export const GAME_SLUG =
  (import.meta.env['VITE_BLOXITY_GAME_ID'] as string | undefined)?.trim() || BLOXITY_GAME_ID;

/**
 * Optional backend overrides, unset in every normal build.
 *
 * With neither set, `init` gets only the slug and the SDK picks its documented
 * defaults (api.bloxity.io / bloxity.io, or the page's own origin on localhost).
 * Setting them lets a LOCAL build log in against a real Bloxity backend.
 */
const API_URL = (import.meta.env['VITE_BLOXITY_API_URL'] as string | undefined)?.trim() || '';
const PORTAL_URL = (import.meta.env['VITE_BLOXITY_PORTAL_URL'] as string | undefined)?.trim() || '';

/**
 * Every portal setting this game answers to. Registering a listener is what
 * makes the control appear in the portal menu, so each key here is wired.
 * `enable_chat` and `background_transparency` are drawn by the portal itself;
 * the game only declares them.
 */
...
/**
 * The SDK, or null. EVERY call goes through here: the script comes from a
...
/** A guarded call that returns a promise: a rejection is logged and becomes undefined. */
const guardAsync = async <T>(what: string, body: (api: LegionSdk) => Promise<T> | undefined): Promise<T | undefined> => {
  try {
    return await guard(what, body);
  } catch (error) {
    logger.warn(SCOPE, `${what} failed: ${String(error)}`);
    return undefined;
  }
...
export class Bloxity {
  ...
  constructor(private readonly host: BloxityHost) {}
  ...
  start(): void {
    ...
    logger.info(SCOPE, `Legion.SDK.init ran as "${GAME_SLUG}" (${this.embedded ? 'embedded' : 'standalone'})`);
  ...
  async setProportions(partial: Partial<LegionProportions>): Promise<void> {
    await guardAsync('avatar.setProportions', (api) => api.avatar?.setProportions?.(partial));
  }

  async resetProportions(): Promise<void> {
    await guardAsync('avatar.resetProportions', (api) => api.avatar?.resetProportions?.());
  }
  ...
  async getFriends(): Promise<LegionFriend[]> {
    return (await guardAsync('social.getFriends', (api) => api.social?.getFriends?.())) ?? [];
  }
  ...
  async inviteFriend(userId: string, roomId: string): Promise<boolean> {
    ...
    return (await guardAsync('social.inviteFriend', (api) => api.social?.inviteFriend?.(userId))) ?? false;
  }
  ...
  async sendFriendRequest(userId: string): Promise<LegionFriendRequestResult> {
    return (
      (await guardAsync('social.sendFriendRequest', (api) => api.social?.sendFriendRequest?.(userId))) ?? {
        success: false,
  ...
  async requestPurchase(sku: string, metadata?: Record<string, unknown>): Promise<LegionPurchaseResult> {
    return (
      (await guardAsync('bux.requestPurchase', (api) => api.bux?.requestPurchase?.(sku, metadata))) ?? {
        success: false,
  ...
  async getBuxBalance(): Promise<number | null> {
    const balance = await guardAsync('bux.getBalance', (api) => api.bux?.getBalance?.());
  ...
  private watchAvatar(): void {
    ...
    this.keep(guard('avatar.onProportionsChanged', (api) => api.avatar?.onProportionsChanged?.(() => this.pushAvatar())));
  }
    ...
    switch (key) {
      ...
      case 'fullscreen':
        guard('portal.fullscreen', (api) => {
          ...
          else if (document.fullscreenElement) api.portal?.exitFullscreen?.();
        });
        break;
      case 'enable_chat':
  ...
  /**
   * Portal player events. The spec's `onEvent` stream is preferred; the
   * reference page's per-event listeners are the fallback, so either SDK build
   * delivers respawn requests.
   */
  private watchPlayerEvents(): void {
    const player = sdk()?.player;
    if (player?.onEvent) {
      this.keep(
        guard('player.onEvent', () =>
    ...
    this.keep(guard('player.onRespawnRequest', () => player?.onRespawnRequest?.(() => this.host.respawn())));
    this.keep(
      guard('player.onChatMessageSent', () =>
        player?.onChatMessageSent?.((message) => logger.info(SCOPE, `chat: ${String(message)}`)),
      ),
    );
  }
}
